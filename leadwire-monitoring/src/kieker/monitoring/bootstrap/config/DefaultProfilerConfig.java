...
package kieker.monitoring.bootstrap.config;

import kieker.common.logging.Log;
import kieker.common.logging.LogFactory;
import kieker.monitoring.bootstrap.resolver.condition.MainClassCondition;
import kieker.monitoring.bootstrap.util.NumberUtils;
import kieker.monitoring.bootstrap.util.spring.PropertyPlaceholderHelper;
import kieker.monitoring.common.util.PropertyUtils;
...
public class DefaultProfilerConfig implements ProfilerConfig {
    ...
    public static ProfilerConfig load(String pinpointConfigFileName) throws IOException {
        try {
            ...
            if (LOG.isWarnEnabled()) {
                LOG.warn(pinpointConfigFileName + " file does not exist. Please check your configuration.");
            }
            throw fe;
        } catch (IOException e) {
            if (LOG.isWarnEnabled()) {
                LOG.warn(pinpointConfigFileName + " file I/O error. Error:" + e.getMessage(), e);
            }
            throw e;
        }
    }
    ...
    public Map<String, String> readPattern(String propertyNamePatternRegex) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.info(propertyNamePatternRegex + "=" + result);
        }
