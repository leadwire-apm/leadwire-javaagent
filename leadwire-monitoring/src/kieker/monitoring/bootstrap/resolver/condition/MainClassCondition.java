...
package kieker.monitoring.bootstrap.resolver.condition;
...
import kieker.monitoring.common.util.SimpleProperty;
import kieker.monitoring.common.util.SystemProperty;
import kieker.monitoring.common.util.SystemPropertyKey;
...
public class MainClassCondition implements Condition<String>, ConditionValue<String> {
    ...
    public MainClassCondition(SimpleProperty property) {
        ...
        if (this.applicationMainClassName == NOT_FOUND) {
        	LOG.info("Main class could not be deduced");
        	LOG.info("If you're running on 1.6.0_24 or prior version of Java, consider upgrading to 1.6.0_25+.");
        }
    }
    ...
    public boolean check(String condition) {
        ...
        if (this.applicationMainClassName.equals(condition)) {
        	LOG.debug("Main class match - ["+this.applicationMainClassName+", "+condition+"]");
            return true;
        } else {
        	LOG.debug("Main class does not match - found : ["+this.applicationMainClassName+"], expected : ["+condition+"]");
            return false;
        }
    }
    ...
    private String getMainClassName(SimpleProperty property) {
        ...
        if (javaCommand.isEmpty()) {
            ...
            LOG.warn("Error retrieving main class using '"+SystemPropertyKey.SUN_JAVA_COMMAND.getKey()+"', jre : "+jreVersion );
            return NOT_FOUND;
        } else {
            ...
            try {
            	...
            	LOG.warn("Error retrieving main class", e);
                return NOT_FOUND;
            } finally {
                if (executableArchive != null) {
                    try {
                    	...
                    	LOG.warn("Error closing jarFile : ["+executableArchive.getName()+"]", e);
                    }
                }
            }
        }
