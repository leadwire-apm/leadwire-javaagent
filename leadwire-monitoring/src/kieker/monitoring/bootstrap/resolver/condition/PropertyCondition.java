...
import kieker.monitoring.common.util.StringUtils;
...
public class PropertyCondition implements Condition<String>, ConditionValue<SimpleProperty> {
	...
	static final Log LOG = LogFactory.getLog(PropertyCondition.class); // NOPMD package for inner class
    ...
    public boolean check(String requiredKey) {
        ...
        if (this.property.getProperty(requiredKey) != null) {
			LOG.debug("Property '"+requiredKey+"' found in ["+this.property.getClass().getSimpleName()+"]");

            return true;
        } else {
            LOG.debug("Property '"+requiredKey+"' not found in ["+this.property.getClass().getSimpleName()+"]");
            return false;
        }
    }
