...
public class ClassResourceCondition implements Condition<String> {
	...
	static final Log LOG = LogFactory.getLog(ClassResourceCondition.class); // NOPMD package for inner class
    ...
    public boolean check(String requiredClass) {
        ...
        if (ClassLoader.getSystemResource(classNameAsResource) != null) {
            LOG.debug("Resource found - ["+classNameAsResource+"]");
            return true;
        } else {
            LOG.debug("Resource not found - ["+classNameAsResource+"]");
            return false;
        }
    }
